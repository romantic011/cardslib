...
/**
 * Card view </p> Use an XML layout file to display it. </p> First, you need an
 * XML layout that will display the Card.
 * 
 * <pre>
 * <code>
 ...
 * </pre>
 * 
 * You can easily build your layout. </p> The quickest way to start with this
 * would be to copy one of this files and create your layout. Then you can
 * inflate your layout in the `CardView` using the attr:
 * `card:card_layout_resourceID="@layout/my_layout` Example:
 * 
 * <pre>
 * <code>
 *      <it.gmariotti.cardslib.library.view.CardView
...
public class CardView extends BaseCardView {
	...
	/**
	 * Listener invoked when Expand Animator starts It is used internally
	 */
	...
	protected void initAttrs(AttributeSet attrs, int defStyle) {
		...
		TypedArray a = getContext().getTheme().obtainStyledAttributes(attrs,
				R.styleable.card_options, defStyle, defStyle);

		try {
			card_layout_resourceID = a.getResourceId(
					R.styleable.card_options_card_layout_resourceID,
					this.card_layout_resourceID);
		} finally {
			a.recycle();
		}
	}
	...
	/**
	 * Add a {@link Card}. It is very important to set all values and all
	 * components before launch this method.
	 * 
	 * @param card
	 *            {@link Card} model
	 */
	...
	/**
	 * Refreshes the card content and replaces the inner layout elements (it
	 * inflates layouts again!)
	 * 
	 * @param card
	 */
	...
	protected void setupHeaderView() {
		...
		if (mCardHeader != null) {
			...
			if (mInternalHeaderLayout != null) {
				...
				mInternalHeaderLayout
						.setForceReplaceInnerLayout(isForceReplaceInnerLayout());
				// Add Header View
	...
	protected void setupMainView() {
		if (mInternalContentLayout != null) {
			...
			if (!isRecycle() || isForceReplaceInnerLayout()) {
				...
				if (isForceReplaceInnerLayout()
						&& mInternalContentLayout != null
						&& mInternalInnerView != null)
					((ViewGroup) mInternalContentLayout)
							.removeView(mInternalInnerView);

				mInternalInnerView = mCard.getInnerView(getContext(),
						(ViewGroup) mInternalContentLayout);
			} else {
				// View can be recycled.
				// Only setup Inner Elements
				if (mCard.getInnerLayout() > -1)
					mCard.setupInnerViewElements(mParentGroup,
							mInternalInnerView);
			}
		}
	}
	...
	protected void setupExpandAction() {
		...
		if (mInternalExpandLayout != null
				&& ((mCardHeader != null && mCardHeader.isButtonExpandVisible()) || mCard
						.getViewToClickToExpand() != null)) {
			...
			mInternalExpandLayout.getViewTreeObserver().addOnPreDrawListener(
					new ViewTreeObserver.OnPreDrawListener() {
						...
						public boolean onPreDraw() {
							...
							View parent = (View) mInternalExpandLayout
									.getParent();
							...
							mInternalExpandLayout
									.measure(widthSpec, heightSpec);

							mExpandAnimator = createSlideAnimator(0,
	...
	protected void setupListeners() {
		...
		if (mCard.isSwipeable()) {
			this.setOnTouchListener(new SwipeDismissViewTouchListener(this,
					mCard,
					new SwipeDismissViewTouchListener.DismissCallbacks() {
						...
						public void onDismiss(CardView cardView, Card card) {
							final ViewGroup vg = (ViewGroup) (cardView
									.getParent());
							if (vg != null) {
		...
		if (mCard.isClickable()) {
			...
			if (!mCard.isMultiChoiceEnabled()) {
				if (mCard.getOnClickListener() != null) {
					...
					// if (!mCard.isSwipeable() && mCard.getOnSwipeListener() ==
					// null) {
					...
					if (mMultipleOnClickListner != null
							&& !mMultipleOnClickListner.isEmpty()) {

						for (int key : mMultipleOnClickListner.keySet()) {
							View viewClickable = decodeAreaOnClickListener(key);
							final Card.OnCardClickListener mListener = mMultipleOnClickListner
									.get(key);
			...
			this.setOnLongClickListener(new OnLongClickListener() {
				...
				public boolean onLongClick(View v) {
					if (mCard.getOnLongClickListener() != null)
						return mCard.getOnLongClickListener().onLongClick(
								mCard, v);
					return false;
				}
			});
	...
	protected View decodeAreaOnClickListener(int area) {
		...
		if (area < Card.CLICK_LISTENER_ALL_VIEW
				&& area > Card.CLICK_LISTENER_CONTENT_VIEW)
			return null;
	...
	protected void setupExpandCollapseActionListener() {
		if (mInternalExpandLayout != null) {
			...
			if (viewToClickToExpand != null) {
				...
				TitleViewOnClickListener titleViewOnClickListener = new TitleViewOnClickListener(
						mInternalExpandLayout, mCard,
						viewToClickToExpand.isViewToSelect());

				if (mCardHeader != null && mCardHeader.isButtonExpandVisible()
						&& mInternalHeaderLayout != null) {
					mInternalHeaderLayout
							.setOnClickExpandCollapseActionListener(titleViewOnClickListener);
				}
				...
				if (viewToClick != null) {
					...
					ViewToClickToExpand.CardElementUI cardElementUI = viewToClickToExpand
							.getCardElementUIToClick();
					if (cardElementUI != null) {
						...
						if (viewToClick != null) {
							viewToClick
									.setOnClickListener(titleViewOnClickListener);
						}
					}
				}
	...
	protected void setupExpandView() {
		if (mInternalExpandLayout != null && mCardExpand != null) {
			...
			if (!isRecycle() || isForceReplaceInnerLayout()) {
				...
				if (isForceReplaceInnerLayout()
						&& mInternalExpandLayout != null
						&& mInternalExpandInnerView != null)
					((ViewGroup) mInternalExpandLayout)
							.removeView(mInternalExpandInnerView);

				mInternalExpandInnerView = mCardExpand.getInnerView(
						getContext(), (ViewGroup) mInternalExpandLayout);
			} else {
				// View can be recycled.
				// Only setup Inner Elements
				if (mCardExpand.getInnerLayout() > -1)
					mCardExpand.setupInnerViewElements(
							(ViewGroup) mInternalExpandLayout,
							mInternalExpandInnerView);
			}

			ViewGroup.LayoutParams layoutParams = mInternalExpandLayout
					.getLayoutParams();
	...
	/**
	 * Listener to expand/collapse hidden Expand Layout It starts animation
	 */
	protected class TitleViewOnClickListener implements View.OnClickListener {
		...
		private void animateExpanding() {
			...
			if (getOnExpandListAnimatorListener() != null) {
				// List Animator
				getOnExpandListAnimatorListener().onExpandStart(
						mCard.getCardView(), mContentParent);
			} else {
				// Std animator
				...
				mExpandAnimator.addListener(new AnimatorListenerAdapter() {
					...
					public void onAnimationEnd(Animator animation) {
						...
						if (mCard.getOnExpandAnimatorEndListener() != null)
							mCard.getOnExpandAnimatorEndListener().onExpandEnd(
									mCard);
					}
				});
		...
		private void animateCollapsing() {
			...
			if (getOnExpandListAnimatorListener() != null) {
				...
				getOnExpandListAnimatorListener().onCollapseStart(
						mCard.getCardView(), mContentParent);
			} else {
				// Std animator
				...
				animator.addListener(new Animator.AnimatorListener() {
					...
					public void onAnimationEnd(Animator animator) {
						...
						if (mCard.getOnCollapseAnimatorEndListener() != null)
							mCard.getOnCollapseAnimatorEndListener()
									.onCollapseEnd(mCard);
					}
	...
	/**
	 * Create the Slide Animator invoked when the expand/collapse button is
	 * clicked
	 */
	...
	/**
	 * Returns the listener invoked when expand/collpase animation starts It is
	 * used internally
	 * 
	 * @return listener
	 */
	...
	/**
	 * Sets the listener invoked when expand/collapse animation starts It is
	 * used internally. Don't override it.
	 * 
	 * @param onExpandListAnimatorListener
	 *            listener
	 */
	public void setOnExpandListAnimatorListener(
			OnExpandListAnimatorListener onExpandListAnimatorListener) {
	 ...
	 * 
	 * @param expanded
	 *            <code>true</code> if the card is expanded
	 */
	...
	/**
	 * Retrieves the InternalMainCardGlobalLayout. Background style is applied
	 * here.
	 * 
	 * @return
	 */
	...
	/**
	 * Changes dynamically the drawable resource to override the style of
	 * MainLayout.
	 * 
		...
		if (drawableResource != null) {
			if (mInternalMainCardLayout != null) {
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN)
					...
					mInternalMainCardLayout
							.setBackgroundDrawable(drawableResource);
			}
		}
	}
}